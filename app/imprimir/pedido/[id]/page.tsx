...
export default async function ImprimirPedido({ params }: Props) {
  ...
  const { data: p } = await supabase
    .from('pedidos')
    .select('id, numero, valor, itens, obs, cliente_id, vendedor_id, empresa_id, cond_pagamento_id, proposta_id, criado_em')
    .eq('id', id)
    .maybeSingle()
  if (!p) notFound()

  // Condição: a do pedido; se faltar, a da proposta de origem
  let condId = p.cond_pagamento_id as string | null
  if (!condId && p.proposta_id) {
    const { data: prop } = await supabase.from('propostas').select('cond_pagamento_id').eq('id', p.proposta_id).maybeSingle()
    condId = (prop?.cond_pagamento_id as string | null) ?? null
  }
  ...
  const [{ data: cliente }, { data: empresa }, { data: cond }, { data: vendedor }] = await Promise.all([
    ...
    p.empresa_id
      ...
      : Promise.resolve({ data: null }),
    condId
      ? supabase.from('cond_pagamentos').select('nome, parcelas, intervalo, entrada, desconto, forma').eq('id', condId).maybeSingle()
      : Promise.resolve({ data: null }),
    p.vendedor_id
      ...
      <div className="max-w-[800px] mx-auto bg-white shadow-sm print:shadow-none p-10 print:p-0 text-gray-800 text-sm">
        ...
        <header className="flex items-start justify-between border-b-2 pb-4" style={{ borderColor: cor }}>
          ...
          <div className="text-right shrink-0">
            <p className="text-lg font-bold" style={{ color: cor }}>PEDIDO</p>
        ...
        <footer className="mt-10 pt-5 border-t border-gray-200">
          <p className="text-sm text-gray-700">Pedido registrado — agradecemos a preferência e ficamos à disposição.</p>
          {vendedor && (
          ...
          <div className="grid grid-cols-2 gap-8 mt-14 text-center text-xs text-gray-500">
            ...
            <div className="border-t border-gray-300 pt-1">Recebido por</div>
          </div>

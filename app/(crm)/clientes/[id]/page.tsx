...
export default async function Cliente360Page({ params }: Props) {
  ...
  const [
    ...
    supabase
      .from('clientes')
      .select('id, nome, empresa, email, telefone, tipo_pessoa, cpf_cnpj, inscricao_estadual, indicador_ie, cep, rua, numero, complemento, bairro, cidade, estado, tipo, segmento, status, valor_total, origem, lead_id, vendedor_maq_id, vendedor_pec_id, parceiro_id, criado_em, atualizado_em')
      .eq('id', id)
      .maybeSingle(),
    supabase
      .from('oportunidades')

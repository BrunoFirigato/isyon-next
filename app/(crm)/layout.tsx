...
export default async function CrmLayout({
  ...
  const { data: tenant } = await supabase
    .from('tenants')
    .select('segmentos, whatsapp_template, email_template_assunto, email_template_corpo, divisao_carteira, aprovacao_pedido, usa_parceiros, tabela_preco_padrao')
    .eq('id', tenantId)
    .maybeSingle()
  ...
  const usaParceiros          = (tenant?.usa_parceiros           as boolean | null) ?? false
  const tabelaPrecoPadrao     = (tenant?.tabela_preco_padrao     as string | null) ?? null
  ...
  return (
    <TenantProvider
      ...
      usaParceiros={usaParceiros}
      tabelaPrecoPadrao={tabelaPrecoPadrao}

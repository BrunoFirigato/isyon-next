...
export default function VendedoresView({ vendedores, currentStatus, currentQ }: Props) {
  ...
  async function handleDelete(id: string) {
    ...
    setDeletingId(null)
    if (error) { toast('Erro ao excluir vendedor', 'error'); return }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Segmentos</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Comissão</th>
              ...
              {vendedores.map((v) => (
                <tr key={v.id} className="hover:bg-gray-50 transition-colors group">
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{formatDate(v.criado_em)}</p>
                  </td>
                  <td className="px-4 py-3">
                    {v.email && <p className="text-gray-600 flex items-center gap-1"><Mail size={12} className="text-gray-400" />{v.email}</p>}
          ...
          {vendedores.map((v) => (
            <div key={v.id} className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div>
                  <p className="font-medium text-gray-900">{v.nome}</p>
                  {v.cargo && <p className="text-xs text-gray-500 mt-0.5">{v.cargo}</p>}
                </div>
              ...
              {(v.email || v.telefone) && (
                <div className="text-sm text-gray-600 space-y-0.5 mb-3">
                  {v.email && <p className="flex items-center gap-1.5"><Mail size={12} className="text-gray-400" />{v.email}</p>}
                  {v.telefone && <p className="flex items-center gap-1.5"><Phone size={12} className="text-gray-400" />{v.telefone}</p>}
                </div>
              )}
      ...
      {deletingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            <h3 className="text-base font-semibold text-gray-900 mb-2">Excluir vendedor?</h3>

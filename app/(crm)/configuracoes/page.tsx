...
export default async function ConfiguracoesPage() {
  ...
  let { data: tenantData, error: tenantErr } = await supabase
    .from('tenants')
    .select('id, nome, plano, status, criado_em, expiracao_contrato, wa_limite, limite_usuarios, segmentos, divisao_carteira, aprovacao_pedido, usa_parceiros, tabela_preco_padrao, proposta_aceite_gera_pedido')
    .eq('id', tenantId)
    .maybeSingle()
  ...
  if (tenantErr) {
    ...
    const fb = await supabase
      ...
      .eq('id', tenantId)
      .maybeSingle()
    tenantData = fb.data ? { ...fb.data, expiracao_contrato: null, wa_limite: null, limite_usuarios: null, divisao_carteira: false, aprovacao_pedido: false, usa_parceiros: false, tabela_preco_padrao: null, proposta_aceite_gera_pedido: false } : null
  }

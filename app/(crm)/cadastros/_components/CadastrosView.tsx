                ...
                {filteredNcms.map((n) => (
                  <div key={n.id} className="p-4 flex items-start justify-between gap-2 group">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">{n.aliq_ipi != null ? `IPI ${n.aliq_ipi}%` : ''}{n.unid_trib ? ` · ${n.unid_trib}` : ''}</p>
                    </div>
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredCfops.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50 group">
                      ...
                      <td className="px-4 py-3"><span className={`text-xs font-medium px-2 py-0.5 rounded-lg ${c.ativo ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{c.ativo ? 'Ativo' : 'Inativo'}</span></td>
                  ...
                  <div key={c.id} className="p-4 flex items-start justify-between gap-2 group">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${c.ativo ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{c.ativo ? 'Ativo' : 'Inativo'}</span>
                      </div>
                      <p className="text-sm text-gray-600 mt-0.5">{c.descricao}</p>
                      <p className="text-xs text-gray-400 mt-0.5">{c.tipo ? (c.tipo === 'saida' ? 'Saída' : 'Entrada') : ''}{c.csosn ? ` · CSOSN ${c.csosn}` : ''}</p>
                    </div>
                  ...
                  {filteredConds.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50 group">
                      <td className="px-4 py-3">
                        ...
                        {c.obs && <p className="text-xs text-gray-400 mt-0.5 truncate max-w-48">{c.obs}</p>}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-lg ${c.ativo ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                ...
                {filteredConds.map((c) => (
                  <div key={c.id} className="p-4 flex items-start justify-between gap-2 group">
                    <div className="flex-1 min-w-0">
                      ...
                      {c.obs && <p className="text-xs text-gray-400 mt-0.5 truncate">{c.obs}</p>}
                    </div>
                    <Actions onEdit={() => { setEditCond(c); setFormOpen(true) }} onDelete={() => openDelete(c.id, 'cond_pagamentos')} />
                  </div>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeletingId(null)} className="flex-1 border border-gray-300 text-gray-700 font-medium py-2.5 rounded-lg text-sm hover:bg-gray-50">Cancelar</button>
...
function Actions({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) {
  return (
    <div className="flex items-center gap-1 justify-end opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
      <button onClick={onEdit} className="p-1.5 rounded-lg hover:bg-blue-50 text-gray-400 hover:text-blue-600 transition-colors" title="Editar">
        <Pencil size={15} />
      </button>
      <button onClick={onDelete} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors" title="Excluir">
        <Trash2 size={15} />
      </button>
    </div>
  )
